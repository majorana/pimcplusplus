...
template<int M, typename ModelType, typename ArgType=double>
...
private:
  ...
  double Chi2 (const Array<ArgType,1> &x, const Array<double,1> &y,
	       const Array<double,1> &sigma, TinyVector<double,M> params);
  void CalcAlphaBeta(const Array<ArgType,1> &x, 
...
public:
  void Fit(const Array<ArgType,1> &x, const Array<double,1> &y,
	   const Array<double,1> &sigma, TinyVector<double,M> &params);
...
template<int M, typename ModelType, typename ArgType> double
NonlinearFitClass<M,ModelType,ArgType>::Chi2
(const Array<ArgType,1> &x,  const Array<double,1> &y,
 const Array<double,1> &sigma, TinyVector<double,M> params)
{
  int N = x.size();
  assert (y.size()     == N);
...
template<int M, typename ModelType, typename ArgType> void
NonlinearFitClass<M,ModelType,ArgType>::CalcAlphaBeta 
(const Array<ArgType,1> &x, const Array<double,1> &y,
 const Array<double,1> &sigma, TinyVector<double,M> params)
{
  Model.SetParams(params);
...
template<int M, typename ModelType, typename ArgType> void
NonlinearFitClass<M,ModelType,ArgType>::Solve()
{
  for (int i=0; i<M; i++) {
    dParams[i] = 0.0;
    for (int j=0; j<M; j++)
...
template<int M, typename ModelType, typename ArgType> void
NonlinearFitClass<M,ModelType,ArgType>::Fit 
(const Array<ArgType,1> &x, const Array<double,1> &y,
 const Array<double,1> &sigma, TinyVector<double,M> &params)
{
