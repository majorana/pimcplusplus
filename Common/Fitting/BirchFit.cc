...
{
  TinyVector<double,ORDER+2> params(80.0, 0.51, 0.0155), errors;  
  BirchEOSClass<ORDER> eos;
  eos.SetParams (params);
  NonlinearFitClass<ORDER+2,BirchEOSClass<ORDER> > fitter(eos);
...
TestFitErrors(string fname)
{
  ...
  for (int i=0; i<numSamples; i++) {
    TinyVector<double,ORDER+2> params2;
    params2 = params;
    BirchEOSClass<ORDER> eos2;
...
void 
StaticFit (string fname, BirchEOSClass<ORDER> &eos)
{
  TinyVector<double,ORDER+2> params(80.0, 0.51, 0.0155, 3.7, 1.8), errors;  
  eos.SetParams (params);
  NonlinearFitClass<ORDER+2,BirchEOSClass<ORDER> > fitter(eos);
  Array<double,1> V, E, sigma;
  ...
  assert(in.ReadVar("B0",  params[2]));
  assert(in.ReadVar("B0p", params[3]));
  assert(in.ReadVar("B0pp", params[3]));
  ...
  fprintf (stdout, "B0   = %12.8f +/- %12.8f\n", 
	   eos.GetB0(), errors[2]);
  fprintf (stdout, "B0FD = %12.8f +/- %12.8f\n", 
	   eos.GetB0FD(), errors[2]);
  fprintf (stdout, "B0p   = %12.8f +/- %12.8f\n", eos.GetB0p(), errors[3]);
  fprintf (stdout, "B0pFD = %12.8f +/- %12.8f\n", eos.GetB0pFD(), errors[3]);
  double V0 = params[0];
  double latConst = cbrt(4.0*V0);
  double latConstError = cbrt(4.0)/(3.0*cbrt(V0)*cbrt(V0)) * errors[0];
  fprintf (stdout, "Lattice const. = %1.6f +/- %1.6f bohr radii\n",
  ...
  fprintf (stdout, "               = %1.6f +/- %1.6f angstrom\n",  
	   0.52917721*latConst, 0.52917721*latConstError);
  stringstream Estr;
  Estr << fname << "_" << ORDER << ".dat";
  string Ename = Estr.str();


  FILE *fout = fopen (Ename.c_str(), "w");
  double delta = (V(0) - V(V.size()-1))/1000.0;
  for (double v=V(V.size()-1); v<=V(0); v+=delta)
    fprintf (fout, "%20.16f %20.16f %20.16f %20.16f %20.16e %20.16e\n", 
	     v, eos(v), eos.Pressure(v), eos.PressureFD(v), 
	     eos.GetB(v), eos.GetBpFD(v));
  fclose (fout);
...
void
CalcProperties (BirchEOSClass<ORDER> &staticEOS,
		DebyeFreeEnergy &thermalEOS)
{
...
main(int argc, char **argv)
{
  if (argc > 2) {
    ...
    cerr << "Usage:  BirchFit2 static.in phonon.dat\n";
}
