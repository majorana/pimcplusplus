...
void ValTest()
{
  ...
  MultiTricubicSpline<numSplines> MultiSpline;
  ...
  TinyVector<double,numSplines> vals2;
  for (double u=0.0; u<=1.0; u+=0.001) {
    ...
    fprintf (f1, "%1.12f ", u);
    fprintf (f2, "%1.12f ", u);
    MultiSpline(x,y,z,vals2);
    for (int n=0; n<numSplines; n++) {
      fprintf (f1, "%1.16e ", MySplines(n)(x,y,z));
      fprintf (f2, "%1.16e ", vals2[n]);
    }
    fprintf (f1, "\n");
    fprintf (f2, "\n");
  }
