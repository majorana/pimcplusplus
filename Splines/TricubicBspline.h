...
TricubicBspline::Find(double x, double y, double z) const 
{
  ...
  tx = modf (xDelta*dx, &xInt);
  ty = modf (xDelta*dy, &yInt);
  tz = modf (xDelta*dz, &zInt);
...
TricubicBspline::operator()(double x, double y, double z)
{
  ...
  a[1]+= A(1,0)*px[1]; a[1]+=A(1,1)*px[1]; a[2]+=A(1,2)*px[1]; a[3]+=A(1,3)*px[1];
  a[2]+= A(2,0)*px[2]; a[1]+=A(2,1)*px[2]; a[2]+=A(2,2)*px[2]; a[3]+=A(2,3)*px[2];
  a[3]+= A(3,0)*px[3]; a[1]+=A(3,1)*px[3]; a[2]+=A(3,2)*px[3]; a[3]+=A(3,3)*px[3];  

  b[0] = A(0,0)*py[0]; b[1] =A(0,1)*py[0]; b[2] =A(0,2)*py[0]; b[3] =A(0,3)*py[0];
  b[1]+= A(1,0)*py[1]; b[1]+=A(1,1)*py[1]; b[2]+=A(1,2)*py[1]; b[3]+=A(1,3)*py[1];
  b[2]+= A(2,0)*py[2]; b[1]+=A(2,1)*py[2]; b[2]+=A(2,2)*py[2]; b[3]+=A(2,3)*py[2];
  b[3]+= A(3,0)*py[3]; b[1]+=A(3,1)*py[3]; b[2]+=A(3,2)*py[3]; b[3]+=A(3,3)*py[3];  

  c[0] = A(0,0)*pz[0]; c[1] =A(0,1)*pz[0]; c[2] =A(0,2)*pz[0]; c[3] =A(0,3)*pz[0];
  c[1]+= A(1,0)*pz[1]; c[1]+=A(1,1)*pz[1]; c[2]+=A(1,2)*pz[1]; c[3]+=A(1,3)*pz[1];
  c[2]+= A(2,0)*pz[2]; c[1]+=A(2,1)*pz[2]; c[2]+=A(2,2)*pz[2]; c[3]+=A(2,3)*pz[2];
  c[3]+= A(3,0)*pz[3]; c[1]+=A(3,1)*pz[3]; c[2]+=A(3,2)*pz[3]; c[3]+=A(3,3)*pz[3];  
