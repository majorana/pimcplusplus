...
ComplexMultiTricubicSpline::UpdateXPeriodic(int source, int dest, int i)
{
  ...
  for (int iy=0; iy<Ny; iy++) 
    for (int iz=0; iz<Nz; iz++) {
      assert (F(0,iy,iz,i)[source][0] == F(M,iy,iz,i)[source][0]);
      assert (F(0,iy,iz,i)[source][1] == F(M,iy,iz,i)[source][1]);
...
ComplexMultiTricubicSpline::UpdateYPeriodic(int source, int dest, int i)
{
  ...
  for (int ix=0; ix<Nx; ix++) 
    for (int iz=0; iz<Nz; iz++) {
      assert (F(ix,0,iz,i)[source][0] == F(ix,M,iz,i)[source][0]);
      assert (F(ix,0,iz,i)[source][1] == F(ix,M,iz,i)[source][1]);
