...
void BisectionMoveClass::makeMove()
{
  ...
  for (int levelCounter=NumLevels;levelCounter>0;NumLevels++){
    ...
    setMode(OLD);
    bool toAccept=true;
    double oldAction = (*PathData).TotalAction.calcTotalAction(ActiveParticles,levelCounter);
    setMode(NEW);
    ...
    double newAction = (*PathData).TotalAction.calcTotalAction(ActiveParticles,levelCounter);
