...
#include "../PathDataClass.h"
	...
	double CEIMCActionClass::SingleAction(int slice1,int slice2,const Array<int,1> &activeParticles,int level){
	  ...
	  //for(int slice=slice1; slice<slice2; slice++){
		...
		if(GetMode() == NEWMODE)
			setPtclSet = ptclSet1;
			//cerr << "  Action: ptclsize is " << activeParticles.size() << " and ModeType is " << GetMode() << endl;
		for(int i=0; i<activeParticles.size(); i++){
		  int ptcl = activeParticles(i);
		  PathData.qmc->SetPtclPos(setPtclSet, ptcl, PathData.Path(slice,ptcl).data());
		  //cerr << "  setting ptcl " << ptcl << " of " << setPtclSet << " to " << PathData.Path(slice,ptcl) << endl;
		}
	
		// only run if we're on NEWMODE
		if(GetMode() == NEWMODE){
...
QMCSamplingClass::QMCSamplingClass(PathDataClass &pathData) : ActionBaseClass (pathData)
{
...
IonIonActionClass::IonIonActionClass(PathDataClass &pathData): ActionBaseClass(pathData){
  prefactor = 1.0;
	cutoff = 1e10;
}

double IonIonActionClass::SingleAction(int slice1,int slice2,const Array<int,1> &activeParticles,int level){
      ...
      int species2 = Path.ParticleSpeciesNum(ptcl2);
			...
			if(Path.DoPtcl(ptcl2) && (species2 != speciese)){
	  		...
	  		for (int slice=slice1;slice<slice2;slice++){
					...
					Utotal += prefactor*PathData.Species(species1).Charge*PathData.Species(species2).Charge*(1.0/r - 1.0/cutoff);
