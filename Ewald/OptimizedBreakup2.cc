...
public:
  double operator()(double r)
  {
    return 4.0*M_PI/(Basis.Omega) *Basis.h(n,r)*r*r;
  }
  c0Integrand(BasisClass &basis, int n_) : Basis(basis), n(n_)
  { /* do nothing */ }
};
...
void OptimizedBreakupClass::SetkVecs(double kc, double kCont, double kMax)
{
  ...
  Vec3 k (0.0, 0.0, 0.0);
  vector<Vec3> Gvecs = Basis.GetLattice().GenGvecs(k, 0.5*kCont*kCont);
  for (int i=0; i<Gvecs.size(); i++) {
    double Gmag2 = dot(Gvecs[i], Gvecs[i]);
    if (Gmag2 > kc*kc) {
      Addk (sqrt(Gmag2));
      numk++;
    }
  ...
  double kvol = (8.0*M_PI*M_PI*M_PI)/Basis.GetLattice().Volume();
  const int N = 4000;
